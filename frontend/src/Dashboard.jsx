...
function ClusteredPhrases({ interviews }) {
  ...
  useEffect(() => {
    ...
    for (const iv of interviews) {
      ...
      if (dominant && byClass[dominant] !== undefined && phrases.length) {
        byClass[dominant].push(...phrases);
      }
    }
    const hasAny = Object.values(byClass).some(arr => arr.length > 0);
    if (!hasAny) { setClustered({}); return; }
    setLoading(true);
    fetch(`${API}/api/cluster-phrases`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ phrases_by_class: byClass })
    })
    .then(r => r.json())
    .then(data => { setClustered(data); setLoading(false); })
    .catch(() => {
      const fallback = {};
      for (const [k, arr] of Object.entries(byClass)) {
        ...
        fallback[k] = Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,10).map(([label,count])=>({label,count}));
      }
        ...
        return (
          <div key={key} style={{ marginBottom: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: '500' }}>{CLASS_LABELS[key] || key}</span>
            ...
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', paddingLeft: '14px' }}>
              {phrases.map((p, i) => {
                ...
                const alpha = Math.round((0.2 + intensity * 0.6) * 255).toString(16).padStart(2, '0');
                const textColor = intensity > 0.5 ? '#fff' : '#333';
                return (
                  <span key={i} style={{ background: color + alpha, color: textColor, fontSize: '12px', padding: '4px 10px', borderRadius: '6px', display: 'inline-flex', alignItems: 'center', gap: '5px', border: `1px solid ${color}30` }}>
