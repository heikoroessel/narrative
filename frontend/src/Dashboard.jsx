...
const CLASS_LABELS = {
  ...
  veraenderung: 'Veränderungsnarrative', beziehung: 'Beziehungsnarrative', erfolg: 'Erfolgs-/Scheiternsnarrative',
};
const CLASS_COLORS = {
...
const CLASS_INFO = {
  erklaerung: { desc: 'Wie die Organisation erklärt, warum Dinge so sind.', examples: ['"Das liegt am Markt"', '"Das hat historische Gründe"'] },
  identitaet: { desc: 'Was die Organisation über sich selbst erzählt.', examples: ['"Wir sind eine Familie"', '"Wir sind Pioniere"'] },
  macht: { desc: 'Wer handelt, wer entscheidet, wer Objekt ist.', examples: ['"Bei uns entscheidet immer..."', '"Wir werden nie gefragt"'] },
  veraenderung: { desc: 'Wie die Organisation mit Wandel umgeht.', examples: ['"Das haben wir schon immer so gemacht"', '"Wir sind im Aufbruch"'] },
  beziehung: { desc: 'Wie über interne und externe Beziehungen gesprochen wird.', examples: ['"Wir gegen die anderen Abteilungen"', '"Unsere Kunden verstehen uns nicht"'] },
  erfolg: { desc: 'Wie Erfolge und Misserfolge erklärt werden.', examples: ['"Das haben wir erreicht weil..."', '"Das ist gescheitert weil die anderen..."'] },
};
const HIERARCHY_LABELS = { geschaeftsleitung: 'Geschäftsleitung', management: 'Mittleres Management', operativ: 'Operative Ebene' };
  ...
  return (
    <span style={{ position: 'relative', display: 'inline-block' }}>
      <button onClick={() => setOpen(!open)}
        style={{ width: '16px', height: '16px', borderRadius: '50%', border: '1px solid #ccc', background: '#f5f4f0', color: '#888', fontSize: '10px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginLeft: '5px', fontFamily: 'inherit', verticalAlign: 'middle' }}>
        i
      </button>
      {open && (
        <div style={{ position: 'absolute', left: '20px', top: '-4px', zIndex: 20, background: '#fff', border: '1px solid #e5e5e0', borderRadius: '10px', padding: '12px 14px', width: '220px', boxShadow: '0 4px 16px rgba(0,0,0,0.1)', fontSize: '12px', lineHeight: '1.6' }}>
          <div style={{ fontWeight: '500', marginBottom: '5px' }}>{CLASS_LABELS[classKey]}</div>
          <div style={{ color: '#666', marginBottom: '7px' }}>{info.desc}</div>
          <div style={{ color: '#999', fontSize: '11px' }}>Typisch:</div>
          {info.examples.map((ex, i) => <div key={i} style={{ color: '#534AB7', fontSize: '11px', marginTop: '2px' }}>{ex}</div>)}
          <button onClick={() => setOpen(false)} style={{ position: 'absolute', top: '6px', right: '8px', background: 'none', border: 'none', cursor: 'pointer', color: '#aaa', fontSize: '14px' }}>×</button>
        </div>
      )}
    </span>
  );
...
function BarChart({ classes }) {
  ...
  return (
    <div>{sorted.map(([key, val]) => (
      <div key={key} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '9px' }}>
        <div style={{ width: '160px', fontSize: '12px', color: '#666', flexShrink: 0, display: 'flex', alignItems: 'center' }}>
...
function ClusteredPhrases({ interviews }) {
  const [clustered, setClustered] = useState(null);
  const [loading, setLoading] = useState(false);

  const buildAndCluster = useCallback(async () => {
    // collect all key_phrases per class from all interviews
    const byClass = { erklaerung: [], identitaet: [], macht: [], veraenderung: [], beziehung: [], erfolg: [] };
    for (const iv of interviews) {
      const dominant = iv.dominant;
      const phrases = iv.key_phrases || [];
      if (dominant && phrases.length) {
        byClass[dominant].push(...phrases);
      }
    }
    const hasAny = Object.values(byClass).some(arr => arr.length > 0);
    if (!hasAny) return;

    setLoading(true);
    try {
      const res = await fetch(`${API}/api/cluster-phrases`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phrases_by_class: byClass })
      });
      const data = await res.json();
      setClustered(data);
    } catch (e) {
      // fallback: just show raw phrases with count 1
      const fallback = {};
      for (const [k, arr] of Object.entries(byClass)) {
        const unique = [...new Set(arr)];
        fallback[k] = unique.slice(0, 10).map(p => ({ label: p, count: arr.filter(x => x === p).length }));
      }
      setClustered(fallback);
    }
    setLoading(false);
  }, [interviews]);

  useEffect(() => { if (interviews?.length > 0) buildAndCluster(); }, [interviews?.length]);

  if (loading) return (
    <div style={{ background: '#fff', border: '1px solid #e5e5e0', borderRadius: '12px', padding: '18px', marginBottom: '16px', color: '#aaa', fontSize: '13px' }}>
      Phrasen werden analysiert…
    </div>
  );
  if (!clustered) return null;

  const hasData = Object.values(clustered).some(arr => arr?.length > 0);
  if (!hasData) return null;

  const maxCount = Math.max(...Object.values(clustered).flat().map(p => p.count || 1));

  return (
    <div style={{ background: '#fff', border: '1px solid #e5e5e0', borderRadius: '12px', padding: '18px', marginBottom: '16px' }}>
      <div style={{ fontSize: '11px', fontWeight: '500', color: '#888', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '16px' }}>Typische Narrative & Phrasen</div>
      {Object.entries(clustered).map(([key, phrases]) => {
        if (!phrases?.length) return null;
        return (
          <div key={key} style={{ marginBottom: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
              <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: CLASS_COLORS[key], flexShrink: 0 }} />
            ...
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', paddingLeft: '14px' }}>
              {phrases.map((p, i) => {
                ...
                const baseColor = CLASS_COLORS[key];
                ...
                return (
                  <span key={i} style={{
                    ...
                    {p.count > 1 && <span style={{ fontSize: '10px', fontWeight: '600', opacity: 0.8 }}>{p.count}×</span>}
                    {p.label}
                  </span>
                );
              })}
...
function InterviewDetail({ id, onClose }) {
  ...
  useEffect(() => {
    fetch(`${API}/api/interviews/${id}`).then(r => r.json()).then(d => { setData(d); setLoading(false); }).catch(() => setLoading(false));
  }, [id]);

  if (loading) return <div style={{ padding: '14px', color: '#aaa', fontSize: '13px' }}>Wird geladen…</div>;
  if (!data) return null;
    ...
    <div style={{ background: '#fafaf8', border: '1px solid #e5e5e0', borderRadius: '10px', padding: '16px', marginTop: '8px' }}>
      ...
      {pieData.length > 0 && (
        <div style={{ height: '170px', marginBottom: '12px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={60} innerRadius={28}>
      ...
      {sorted.map(([key, val]) => (
        <div key={key} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '7px' }}>
          <div style={{ width: '150px', fontSize: '11px', color: '#666', flexShrink: 0 }}>{CLASS_LABELS[key]}</div>
...
export default function Dashboard({ orgId }) {
  ...
  const pieData = Object.entries(data.aggregate_classes).filter(([,v]) => v > 0).sort((a,b) => b[1]-a[1]).map(([k,v]) => ({ name: CLASS_LABELS[k], value: v, color: CLASS_COLORS[k] }));
  ...
  return (
    <div>
      ...
      {/* Clustered phrases */}
      <ClusteredPhrases interviews={data.interviews} />
