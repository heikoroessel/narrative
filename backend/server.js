...
const SYSTEM_PROMPT = `Du führst ein narratives Interview mit einer Mitarbeitenden einer Organisation.

Deine Rolle: neutrale, aufmerksame Gesprächsbegleiterin. Kein Coach, kein Berater, kein Therapeut.

WICHTIG: Die Person hat bereits eine Einführung erhalten und eine Erlebniskurve gezeichnet. Starte SOFORT mit einer einladenden Eröffnungsfrage ohne Begrüssung oder Erklärungen. Lade die Person ein, ihre Geschichte zu beginnen. Zum Beispiel: "Schauen Sie auf Ihre Kurve — wie war der Anfang? Was haben Sie erlebt, als Sie hier angefangen haben?"

VERHALTENSREGELN:
- Ausschliesslich Fragen zu Themen, die die Person selbst angesprochen hat
- KEINE Warum-Fragen
- NICHTS bewerten
- Keine Hypothesen, keine Interpretationen, keine Ratschlaege
- Nie nach vollstaendigen Namen fragen
- Falls Namen genannt: einmalig freundlich darauf hinweisen

ERLAUBTES FRAGEREPERTOIRE:
1. "Und dann?", "Wie ging das weiter?"
2. "Was war das fuer eine Phase?", "Was hat sich da veraendert?"
3. "Haben Sie ein Beispiel dafuer?", "Woran haben Sie das gemerkt?"

Antworte auf Deutsch. Max. 2-3 Saetze.`;
...
app.get('/api/dashboard', async (req, res) => {
  ...
  // Collect and cluster phrases per class
  const byClass = { erklaerung: [], identitaet: [], macht: [], veraenderung: [], beziehung: [], erfolg: [] };
  for (const row of rows) {
    const d = row.narrative_classes?.dominant;
    const phrases = row.narrative_classes?.key_phrases || [];
    if (d && byClass[d] && phrases.length) byClass[d].push(...phrases);
  }

  let clustered_phrases = {};
  const hasAny = Object.values(byClass).some(a => a.length > 0);
  if (hasAny) {
    try {
      const CLUSTER_PROMPT = `Du bekommst Phrasen aus narrativen Interviews pro Kommunikationsklasse.
Fasse semantisch ähnliche Phrasen zu einem Label zusammen. Lass unterschiedliche Gedanken getrennt.
Zähle wie oft jede Gruppe vorkommt. Sortiere nach Häufigkeit absteigend. Max. 10 pro Klasse.
Antworte NUR mit JSON (kein Markdown, keine Erklärung):
{"erklaerung":[{"label":"...","count":1}],"identitaet":[...],"macht":[...],"veraenderung":[...],"beziehung":[...],"erfolg":[...]}`;
      const r = await anthropic.messages.create({
      ...
      clustered_phrases = JSON.parse(text);
    } catch (err) {
      console.error('Cluster error:', err.message);
      // fallback: simple count without clustering
      for (const [k, arr] of Object.entries(byClass)) {
        if (!arr.length) continue;
        const counts = {};
        for (const p of arr) counts[p] = (counts[p] || 0) + 1;
        clustered_phrases[k] = Object.entries(counts).sort((a,b)=>b[1]-a[1]).slice(0,10).map(([label,count])=>({label,count}));
      }
    }
  }

  res.json({
    total: rows.length, aggregate_classes: agg, dominant, clustered_phrases,
...
// Cluster phrases for dashboard
app.post('/api/cluster-phrases', async (req, res) => {
  const { phrases_by_class } = req.body;
  if (!phrases_by_class) return res.status(400).json({ error: 'phrases_by_class required' });

  const CLUSTER_PROMPT = `Du bekommst eine Liste von Phrasen pro Kommunikationsklasse aus narrativen Interviews.
Deine Aufgabe: Fasse semantisch ähnliche Phrasen zu einem Begriff zusammen. Lass unterschiedliche Gedanken getrennt.
Zähle wie oft jede Gruppe vorkommt (Summe der zusammengefassten Phrasen).
Sortiere nach Häufigkeit absteigend. Max. 10 Gruppen pro Klasse.

Antworte NUR mit JSON (kein Markdown):
{
  "erklaerung": [{"label": "kurze Zusammenfassung", "count": 3}],
  "identitaet": [...],
  "macht": [...],
  "veraenderung": [...],
  "beziehung": [...],
  ...
  try {
    const r = await anthropic.messages.create({
      ...
      messages: [{ role: 'user', content: `${CLUSTER_PROMPT}\n\nPHRASEN:\n${JSON.stringify(phrases_by_class, null, 2)}` }]
    });
    const clustered = JSON.parse(r.content[0].text.trim());
    res.json(clustered);
  } catch (err) {
    console.error('Cluster error:', err.message);
    res.status(500).json({ error: 'Clustering fehlgeschlagen' });
  }
});
